...
// ── TIER BADGES ──
const TIER_CONFIG = {
    "STRATEGIC_BUYER":   { label: "STRATEGIC",  color: "#2d6a4f", bg: "rgba(45,106,79,0.1)" },
    "FINANCIAL_SPONSOR": { label: "FINANCIAL",  color: "#86BC25", bg: "rgba(134,188,37,0.1)" },
    "ADJACENT_SYNERGY":  { label: "SYNERGY",    color: "#4a6fa5", bg: "rgba(74,111,165,0.1)" },
};
...
async function fetchDealIntel(targetTicker, targetName, targetIndustry, targetRevenue, comps) {
    ...
    try {
        ...
        if (btn) {
            btn.innerHTML = "🧠 DEAL INTELLIGENCE ✓";
            btn.disabled = false;
        ...
        if (btn) {
            btn.innerHTML = "🧠 DEAL INTELLIGENCE (error)";
            btn.disabled = false;
...
function renderDealIntelRows() {
    ...
    rows.forEach(row => {
        ...
        const approachCfg = APPROACH_CONFIG[brief.approach_rec] || APPROACH_CONFIG["SECONDARY"];
...
function toggleDealBrief(ticker, row) {
    ...
    const tierCfg = TIER_CONFIG[brief.tier] || TIER_CONFIG["STRATEGIC_BUYER"];
    const approachCfg = APPROACH_CONFIG[brief.approach_rec] || APPROACH_CONFIG["SECONDARY"];
    const signalCfg = SIGNAL_CONFIG[brief.expansion_signal] || SIGNAL_CONFIG["MEDIUM"];
