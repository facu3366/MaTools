...
async function loadEmpresas() {
  try {
    ...
    console.log("Empresas cargadas:", EMPRESA_LIST.length);
  } catch (err) {
...
onmousedown="selectSuggestion('${e.ticker}', '${e.name.replace(/'/g, "\\'")}', '${e.sector}')"           onmouseover="highlightSuggestion(${i})">
...
async function selectSuggestion(ticker, name, sector) {
  ...
  // auto seleccionar sector desde JSON
...
async function fetchRevenueCard(ticker, name) {
  console.log("===== fetchRevenueCard START =====");
  ...
  try {
    ...
    container.innerHTML = `<div class="rev-error">Error: ${err.message}</div>`;
  }
